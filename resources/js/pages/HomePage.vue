        ...
        <main>
            <section class="new-hero">
                ...
                <div class="hero-product" aria-label="Aperçu illustratif du produit AISYSPRO">
                    <div class="product-window"><header><i></i><i></i><i></i><small>workspace.aisyspro.tn</small></header><div class="product-main"><div class="product-title"><span><small>Vue d’ensemble</small><b>Bonjour, équipe AISYSPRO 👋</b></span><button>+ Nouveau devis</button></div><div class="product-kpis"><article><small>Nouveaux leads</small><b>48</b><em>↗ 18,2%</em></article><article><small>Devis ouverts</small><b>17</b><em>↗ 6,4%</em></article><article><small>Conversion</small><b>68%</b><em>↗ 4,1%</em></article></div><div class="product-panels"><article><b>Performance commerciale</b><div class="mini-chart"><i v-for="height in [35, 48, 43, 68, 58, 81, 95]" :key="height" :style="{ height: `${height}%` }"></i></div></article><article><b>Pipeline</b><p>Nouveau <strong>48</strong></p><p>Qualifié <strong>32</strong></p><p>Proposition <strong>26</strong></p><p>Gagné <strong>8</strong></p></article></div></div></div>
                </div>
            </section>

            <section class="signal-strip"><span>CRM MÉTIER</span><span>SITE WEB</span><span>CMS</span><span>HÉBERGEMENT</span><span>AUTOMATISATION</span></section>

            <section class="new-section ecosystem"><div class="new-heading"><span class="eyebrow-new">Une vision 360°</span><h2>Trois expertises.<br><em>Un seul partenaire responsable.</em></h2><p>Plus de rupture entre votre vitrine, vos prospects et vos opérations : tout communique, tout se mesure.</p></div><div class="expertise-grid"><article><span>01</span><h3>CRM & application métier</h3><p>Votre vocabulaire, vos règles, vos documents et vos indicateurs dans une plateforme qui ressemble à votre entreprise.</p><RouterLink to="/solutions">Explorer le CRM ↗</RouterLink></article><article><span>02</span><h3>Site web connecté</h3><p>Une présence rapide, crédible et orientée conversion. Chaque formulaire alimente le CRM automatiquement.</p><RouterLink to="/packs">Découvrir le web ↗</RouterLink></article><article><span>03</span><h3>Hébergement géré</h3><p>Domaine, SSL, emails, sauvegardes et supervision réunis dans une infrastructure suivie par nos experts.</p><RouterLink to="/packs">Voir l’infrastructure ↗</RouterLink></article></div></section>

            <section class="new-section solutions-preview"><div class="new-heading"><span class="eyebrow-new">Une solution pour chaque métier</span><h2>Votre activité change.<br><em>Le système s’adapte.</em></h2><p>Les solutions prioritaires et leur ordre sont pilotés directement depuis le Back Office.</p></div><div class="sector-new-grid"><RouterLink v-for="(solution, index) in content.solutions" :key="solution.id" :to="`/solutions/${solution.slug}`" class="sector-card"><span>{{ String(index + 1).padStart(2, '0') }}</span><small>{{ solution.category }}</small><h3>{{ solution.title }}</h3><p>{{ solution.short_description }}</p><div><em v-for="module in solution.modules?.slice(0, 3)" :key="module">{{ module }}</em></div><b>Découvrir ↗</b></RouterLink><RouterLink to="/solutions" class="sector-card discover-card"><span>06</span><small>Catalogue complet</small><h3>Plus de 50 solutions métier</h3><p>Explorez les solutions adaptées aux secteurs et processus professionnels.</p><b>Explorer les métiers ↗</b></RouterLink></div></section>
            ...
            <section class="new-section blog-preview"><div class="new-heading"><span class="eyebrow-new">Conseils & expertise</span><h2>Décider avec<br><em>les bonnes informations.</em></h2><p>Des guides sectoriels concrets pour cadrer vos priorités, vos processus et votre transformation digitale.</p></div><div class="blog-catalog-grid"><article v-for="post in content.posts" :key="post.id" class="blog-card"><div class="blog-cover"><span>{{ post.category?.charAt(0) || 'A' }}</span></div><small>{{ post.category }}</small><h3>{{ post.title }}</h3><p>{{ post.excerpt }}</p><RouterLink :to="`/blog/${post.slug}`">Lire le guide ↗</RouterLink></article></div><RouterLink to="/blog" class="text-link">Voir tous les guides métier →</RouterLink></section>
        </main>
    </PublicLayout>
</template>
